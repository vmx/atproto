...
//      `integer`. `recordSchema.parse(...)` walks the input through the
...
//      the lexicon pass through `parse` unchanged. The CBOR encoder then
...
// `parse` runs each schema-typed field through its validator (wrapping bare
...
// ---------------------------------------------------------------------------
// 5. Failure paths. Two producer-side rejections, both ensuring a
//    non-integer never reaches the wire as an integer-typed value.
// ---------------------------------------------------------------------------
...
console.log('Schema rejects a non-integer at an integer-typed field:')
try {
  recordSchema.parse({ ...input, sampleCount: 3.14 })
} catch (err) {
  console.log(`  ${err.constructor.name}: ${err.message}`)
}
