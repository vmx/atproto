 ...
 * float.
 *
 * The constructor throws `TypeError` on `NaN` and `Infinity`: those values
 * have no AT data model representation, so a `LexFloat` wrapping them could
 * never be encoded. Failing at construction matches the symmetric assertion
 * that {@link LexInteger} performs for safe integers.
 *
 * Producers:
...
export class LexFloat {
  constructor(readonly value: number) {
    if (typeof value !== 'number' || !Number.isFinite(value)) {
      throw new TypeError(`LexFloat requires a finite number, got ${value}`)
    }
  }
}
