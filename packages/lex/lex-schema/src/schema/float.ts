...
export class FloatSchema extends Schema<number> {
  ...
  validateInContext(input: unknown, ctx: ValidationContext) {
    ...
    // In parse / coerce mode, coerce bare numbers to `LexFloat` so the JSON
    // serialiser emits the float-tagged form (`65.0`). Validate mode returns
    // the input unchanged — wrapping there would trip the "value changed"
    // guard in ValidationContext and fail the validation.
    if (ctx.options.mode !== 'validate' && !isLexFloat(input)) {
